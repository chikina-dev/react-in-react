...
test("replacing sample1.zip with sample2.zip and sample3.zip keeps preview state aligned", async ({
  page,
}) => {
  ...
  await expect(page.getByText(/sample2\.zip into \/workspace\./)).toHaveCount(0);
  await page.getByRole("button", { name: "Use npm run dev" }).click();
  await page.getByRole("button", { name: "Run session" }).click();

  const thirdPreviewUrl = await waitForPreviewUrl(page);
  expect(thirdPreviewUrl).toContain("/preview/");
  expect(thirdPreviewUrl).not.toBe(secondPreviewUrl);
  await expect(previewFrame.getByText("What's next?")).toBeVisible({ timeout: 60000 });
  await expect(previewFrame.getByText("React Router Docs")).toBeVisible({ timeout: 60000 });
  await expect(previewFrame.getByText("Learn React")).toHaveCount(0);
  await expect(previewFrame.getByText("Get started")).toHaveCount(0);
  await expect(previewFrame.getByText("Preview diagnostics")).toHaveCount(0);
});
